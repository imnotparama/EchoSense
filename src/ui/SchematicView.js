export class SchematicView {
    ...
    this.container.innerHTML = `
      <div class="schematic-header">
        ...
        <div class="sch-actions">
          <button class="sch-tool-btn" id="btn-sch-zoom-in" title="Zoom In">+</button>
          <button class="sch-tool-btn" id="btn-sch-zoom-out" title="Zoom Out">−</button>
          <button class="sch-tool-btn" id="btn-sch-reset" title="Reset Pan & Zoom">1:1</button>
          <button class="sch-tool-btn" id="btn-sch-export" title="Export Vector SVG">📥 SVG</button>
          <button class="sch-close-btn" id="btn-close-schematic">&times;</button>
        </div>
      </div>
      <div class="schematic-canvas-wrap" id="sch-canvas-wrap" style="cursor: grab; overflow: hidden; user-select: none;">
        <svg id="schematic-svg" viewBox="0 0 800 600" xmlns="http://www.w3.org/2000/svg" style="width: 100%; height: 100%;">
          <!-- STYLES -->
          <defs>
          ...
          <g id="schematic-viewport" transform="translate(0, 0) scale(1)">
            <!-- 1. ESP32-S3 BLOCK -->
      ...
      <div class="schematic-breadcrumb" id="sch-breadcrumb">
        <span>Click any schematic symbol or pin to isolate circuit & trace in 3D • Scroll to zoom, drag to pan</span>
      </div>
    `;
  }

  updateTransform() {
    const vp = document.getElementById('schematic-viewport');
    if (vp) {
      vp.setAttribute('transform', `translate(${this.panX}, ${this.panY}) scale(${this.zoomScale})`);
    }
  }
  ...
  bindEvents() {
    ...
    // Zoom buttons
    document.getElementById('btn-sch-zoom-in')?.addEventListener('click', () => {
      this.zoomScale = Math.min(2.5, this.zoomScale * 1.25);
      this.updateTransform();
    });

    document.getElementById('btn-sch-zoom-out')?.addEventListener('click', () => {
      this.zoomScale = Math.max(0.4, this.zoomScale / 1.25);
      this.updateTransform();
    });

    document.getElementById('btn-sch-reset')?.addEventListener('click', () => {
      this.zoomScale = 1.0;
      this.panX = 0;
      this.panY = 0;
      this.updateTransform();
    ...
    // Export SVG button
    document.getElementById('btn-sch-export')?.addEventListener('click', () => {
      ...
      link.href = url;
      link.download = 'EchoSense_Schematic.svg';
    ...
    if (wrap) {
      wrap.addEventListener('wheel', e => {
        ...
        this.zoomScale = Math.min(2.8, Math.max(0.4, this.zoomScale * factor));
        this.updateTransform();
      });
